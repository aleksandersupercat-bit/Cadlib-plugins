...
LOG_PATH = Path.home() / "Desktop" / "set_selected_ms_parameter_log.txt"
TARGET_PARAMETER = "PART_WEIGHT"
TARGET_VALUE = "100"
...
def popup(message):
    try:
        ...
        shell.Popup(message, 5, "DTMX MS parameter writer", 64)
    except Exception as ex:
        log(f"Popup failed: {ex}")


reset_log()
log("=== set_selected_ms_parameter.py start ===")
log(f"Log path: {LOG_PATH}")
log(f"Target parameter: {TARGET_PARAMETER}")
log(f"Target value: {TARGET_VALUE}")
log(f"Target entities: {sorted(ENTITY_NAMES)}")
...
if doc is None:
    ...
    popup("DTMX: no active document. Check set_selected_ms_parameter_log.txt")
else:
    ...
    for index, entity in enumerate(target_entities):
        ...
        if index < PREVIEW_LIMIT:
            log(
                "ITEM[{0}] Handle={1} | EntityName={2} | param_index={3} | before_value={4} | before_get={5}".format(
                    index, handle, entity_name, param_index, before_value, before_get
                )
            )
        ...
        if index < PREVIEW_LIMIT:
            ...
            log(
                "ITEM[{0}] Handle={1} | after_value={2} | after_get={3}".format(
...
log("=== set_selected_ms_parameter.py end ===")
